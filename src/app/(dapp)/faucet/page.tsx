      ...
      <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 pt-20 pb-6 sm:py-24 space-y-6">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto">
            Get testnet tokens for development and testing. Each wallet can mint
            up to 10,000 tokens per token.
          </p>
        </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {Object.keys(FAUCET_TOKENS).map((tokenKey) => (
                    <TokenCard
                      ...
                      onClick={() =>
                        setSelectedToken(tokenKey as keyof typeof FAUCET_TOKENS)
                      }
                    />
                  ))}
                </div>
            ...
            <Card className="bg-white/10 border-gray-800 rounded-lg">
              ...
              <CardContent className="p-4 space-y-4">
                ...
                <div className="flex justify-end p-4">
                  <div className="text-right">
                    <span className="text-gray-400 px-2">Your Balance:</span>
                    <span className="text-gray-400 font-medium">
                      {isLoadingBalance ? (
                        <Loader2 className="w-4 h-4 animate-spin inline" />
                      ) : (
                        `${Number(balance).toLocaleString()} ${token.symbol}`
                      )}
                    </span>
                  </div>
                </div>
