...
const CreatePool: React.FC = () => {
  ...
  const { writeContract: writePoolContract, data: poolHash } =
    useWriteContract();
  const { writeContract: writeLiquidityContract, data: liquidityHash } =
    useWriteContract();

  // Track pool creation transaction
  const { data: poolReceipt, isPending: isPoolReceiptPending } =
    useWaitForTransactionReceipt({
      hash: poolHash,
    });

  // Track liquidity addition transaction
  const { data: liquidityReceipt, isPending: isLiquidityReceiptPending } =
    useWaitForTransactionReceipt({
      hash: liquidityHash,
    });
    ...
    if (!formData.startingPrice) {
      ...
      if (isNaN(price) || price <= 0) {
        ...
        errors.startingPrice =
          "Starting price must be between 0.000001 and 1,000,000";
        isValid = false;
      }
    }
  ...
  const calculateSqrtPriceX96 = (startingPrice: number) => {
    ...
    try {
      ...
      const clampedPrice = Math.max(
        minPrice,
        Math.min(maxPrice, startingPrice)
      );
      ...
      ); // sqrt(1.0001^887272) * 2^96

      const boundedSqrtPriceX96 =
  ...
  const validatePoolKey = (poolKey: CreatePoolConfig["poolKey"]) => {
    ...
    if (!validFees.includes(poolKey.fee)) {
      throw new Error(
        `Invalid fee tier: ${poolKey.fee}. Valid fees are: ${validFees.join(
          ", "
        )}`
      );
    }
  ...
  const approvePermit2ForPositionManager = async (tokenAddress: string) => {
    ...
    try {
      const txHash = await writeContractAsync({
        ...
        abi: Permit2ABIBase,
        functionName: "approve",
        args: [tokenAddress, positionManagerAddress, MAX_UINT160, deadline],
      });
      return txHash;
    } catch (error) {
  ...
  const checkTokenApprovals = async (
    ...
    try {
      ...
      const permit2Expiration = (
        permit2Allowance as [bigint, bigint, bigint]
      )[1];
  ...
  const createPoolConfiguration = () => {
    ...
    if (BigInt(selectedPair.currency0) < BigInt(selectedPair.currency1)) {
      ...
      sortedPoolConfig.startingPrice = calculateSqrtPriceX96(
      ...
      sortedPoolConfig.startingPrice = calculateSqrtPriceX96(
        1 / parseFloat(formData.startingPrice)
      );
  ...
  const handleCheckPool = async () => {
    try {
      ...
      setSubmitError(
        error instanceof Error ? error.message : "Error checking pool"
      );
    } finally {
  ...
  const handleCreatePool = async () => {
    try {
      ...
      const amount0 = parseUnits(
        sortedPoolConfig.depositAmounts0,
        parseInt(sortedPoolConfig.decimals0)
      );
      const amount1 = parseUnits(
        sortedPoolConfig.depositAmounts1,
        parseInt(sortedPoolConfig.decimals1)
      );

      if (balance0 < amount0) {
        setSubmitError(
          `Insufficient ${
            sortedPoolConfig.tokenSymbols0
          } balance. Required: ${formatBalance(
            amount0,
            sortedPoolConfig.decimals0
          )}, Available: ${formatBalance(balance0, sortedPoolConfig.decimals0)}`
        );
        return;
      }
      if (balance1 < amount1) {
        setSubmitError(
          `Insufficient ${
            sortedPoolConfig.tokenSymbols1
          } balance. Required: ${formatBalance(
            amount1,
            sortedPoolConfig.decimals1
          )}, Available: ${formatBalance(balance1, sortedPoolConfig.decimals1)}`
        );
        return;
      }

      // Check and handle token approvals for both currencies
      const currency0Approvals = await checkTokenApprovals(
        poolConfig.poolKey.currency0,
        sortedPoolConfig.decimals0
      );
      const currency1Approvals = await checkTokenApprovals(
        poolConfig.poolKey.currency1,
        sortedPoolConfig.decimals1
      );

      // Approve ERC20 for Permit2 if needed
      const amount0ForApproval = parseUnits(
        sortedPoolConfig.depositAmounts0,
        parseInt(sortedPoolConfig.decimals0)
      );
      ...
      const amount1ForApproval = parseUnits(
        sortedPoolConfig.depositAmounts1,
        parseInt(sortedPoolConfig.decimals1)
      );
      ...
      console.log(
        "poolConfig.poolKey.startingPrice",
        poolConfig.poolKey.startingPrice
      );
      ...
      setSubmitError(
        error instanceof Error ? error.message : "Error creating pool"
      );
  ...
  const handleAddLiquidity = async () => {
    try {
      ...
      if (
        (refetchedSlot0 &&
          Array.isArray(refetchedSlot0) &&
          refetchedSlot0[0] !== 0n) ||
        refetchedLiquidity !== 0n
      ) {
        setIsAddingLiquidity(true);
        console.log("refetchedSlot0", refetchedSlot0);
        console.log("refetchedLiquidity", refetchedLiquidity);
      ...
      const currency0Details = new Token(
        chainId,
        poolConfig.poolKey.currency0,
        parseInt(sortedPoolConfig.decimals0),
        sortedPoolConfig.tokenNames0,
        sortedPoolConfig.tokenSymbols0
      );
      const currency1Details = new Token(
        chainId,
        poolConfig.poolKey.currency1,
        parseInt(sortedPoolConfig.decimals1),
        sortedPoolConfig.tokenNames1,
        sortedPoolConfig.tokenSymbols1
      );
      ...
      const amount0 = parseUnits(
        amount0Raw.toString(),
        currency0Details.decimals
      );
      const amount1 = parseUnits(
        amount1Raw.toString(),
        currency1Details.decimals
      );
  ...
  const fetchTokenBalances = useCallback(
    async (pairIndex?: string) => {
      console.log(
        "fetchTokenBalances called with pairIndex:",
        pairIndex,
      ...
      <div className="w-full px-4 py-8">
        ...
        <Card className="w-full mx-auto border-gray-800 bg-white/5 dark:bg-gray-900/50 backdrop-blur-sm">
          <CardHeader className="space-y-1 p-6">
            ...
            <p className="text-sm text-gray-300 dark:text-gray-400">
              Select your token pair, set the starting price, and specify
              deposit amounts
            </p>
          </CardHeader>
            ...
            <form className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                >
                  <div className="relative">
                    <select
                      ...
                      className={`${
                        formErrors.selectedPair ? "border-red-500" : ""
                    ...
                    {(tokenSymbols.currency0 ||
                      ...
                      isLoadingSymbols.currency0 ||
                      isLoadingSymbols.currency1) && (
                ...
                >
                  <Input
                    ...
                    step="0.000001"
                    className={`${
                      formErrors.startingPrice ? "border-red-500" : ""
                    } bg-white/50 dark:bg-gray-800/50 transition-colors focus:ring-2 focus:ring-blue-500
                ...
                <div className="bg-gray-800/50 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-gray-400">
                      Balance:{" "}
                      {formatBalance(
                        BigInt(tokenBalances.currency0),
                        tokenDecimals.currency0
                      )}{" "}
                      {tokenSymbols.currency0}
                    </span>
                  </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center">
                        <span className="text-xs font-bold text-white">
                          {tokenSymbols.currency0
                            ? tokenSymbols.currency0[0]
                            : "?"}
                        </span>
                      </div>
                      <span className="text-sm text-gray-300">
                        {tokenSymbols.currency0 || "Token"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-800/50 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-gray-400">
                      Balance:{" "}
                      {formatBalance(
                        BigInt(tokenBalances.currency1),
                        tokenDecimals.currency1
                      )}{" "}
                      {tokenSymbols.currency1}
                    </span>
                  </div>
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="w-6 h-6 bg-green-500 rounded-full flex items-center justify-center">
                        <span className="text-xs font-bold text-white">
                          {tokenSymbols.currency1
                            ? tokenSymbols.currency1[0]
                            : "?"}
                        </span>
                      </div>
                      <span className="text-sm text-gray-300">
                        {tokenSymbols.currency1 || "Token"}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">
                        {tokenSymbols.currency0}/{tokenSymbols.currency1}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`font-medium ${
                            poolExists ? "text-green-400" : "text-yellow-400"
                          }`}
                        >
              ...
              <div className="flex justify-center gap-4 pt-4">
                <Button
                  ...
                  className={`px-6 py-3 text-lg font-medium relative overflow-hidden transition-all duration-300 ${
                    isCheckingPool || !formData.selectedPair
                      ? "bg-gray-400"
                ...
                <Button
                  type="button"
                  onClick={
                    poolExists === false ? handleCreatePool : handleAddLiquidity
                  }
                  disabled={
                    isLoading ||
                    poolExists === null ||
                  ...
                  className={`px-8 py-3 text-lg font-medium relative overflow-hidden transition-all duration-300 ${
                    isLoading ||
                    poolExists === null ||
                    isCreatingPool ||
                    isAddingLiquidity
        ...
        {poolReceipt && poolReceipt.status === "success" && (
          <div className="flex items-center p-3 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 rounded-lg">
            ...
            <span className="text-sm">
              <a
                href={getTxnExplorerUrl(poolReceipt.transactionHash, chainId)}
                target="_blank"
        ...
        {liquidityReceipt && liquidityReceipt.status === "success" && (
          <div className="flex items-center p-3 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 rounded-lg">
            ...
            <span className="text-sm">
              <a
                href={getTxnExplorerUrl(
                  liquidityReceipt.transactionHash,
                  chainId
                )}
                target="_blank"
                rel="noopener noreferrer"
        ...
        {/* Error Status */}
        {!isPoolReceiptPending &&
          !isLiquidityReceiptPending &&
          !isCreatingPool &&
          !isAddingLiquidity &&
          !poolReceipt &&
          !liquidityReceipt &&
